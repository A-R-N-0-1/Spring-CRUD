...
public class MedController {
	...
	public String deleteUserById(@PathVariable("id") Integer id, Model model) {
		...
		if (userOptional.isPresent()) {
			...
			return "delete"; // or handle the case where the user with the specified ID does not exist
		}
	}

	@PostMapping("/search")
	public String searchMedicineByName(@RequestParam("keyword") String keyword, Model model) {
		List<Meddetail> users = userRepository.findByMedicinenameContaining(keyword);
		model.addAttribute("users", users);
		return "list";
	}
	...
	@PostMapping("/add")
	public String addUser(Meddetail user) {
